...
class Interface(interface.Interface):
    ...
    def __init__(self, params=None): 
        ...
        self.timeout_interval = 10 # Time after which the background thread should stop if no iteration happens
        ...
        self.last_values = [{"device":"R101", "chan":"CONTROL_AMP", "val":0.0},
                            {"device":"R101", "chan":"CONTROL_AMP_24MHZ", "val":0.0},
                            {"device":"R101", "chan":"CONTROL_AMP_36MHZ", "val":0.0},
                            {"device":"R101", "chan":"CONTROL_AMP_48MHZ", "val":0.0},
                            {"device":"R101", "chan":"CONTROL_PHASE_24MHZ", "val":0.0},
                            {"device":"R101", "chan":"CONTROL_PHASE_36MHZ", "val":0.0},
                            {"device":"R101", "chan":"CONTROL_PHASE_48MHZ", "val":0.0},
                            {"device":"R101", "chan":"PHASEB", "val":0.0}
                            ]
                              # Initialize with default or zeroes
        # Variables to track maximum true_rate and corresponding post_dict
    ...
    def wait_for_new_file(self, histo_file, timeout=10):
        """
        Waits for the histogram file to be updated or created.
        """
        if os.path.exists(histo_file):
            last_mod_time = os.path.getmtime(histo_file)
        else:
            last_mod_time = 0
        ...
        while time.time() - start_time < timeout:
            if os.path.exists(histo_file) and os.path.getmtime(histo_file) > last_mod_time:
                print(f"New histogram file detected: {histo_file}")
                return True

            print("Waiting for histogram file...")
            time.sleep(0.5)

        print("Timeout reached, histogram file not updated.")
        return False
    ...
    def get_values(self, input_list):
        ...
        command = "./N957Demo"  # Your external program command
        histo_file = '/Users/user/Library/Application Support/Badger/plugins/interfaces/histo.dat'  # Histogram file path
        
        # Step 1: Execute the external program
        print(f"Executing: {command}")
        try:
            subprocess.call(command, shell=True)  # This will generate histo.dat
            print(f"Program executed successfully, waiting for file: {histo_file}")
            time.sleep(1)  # Optional delay if the program needs time to generate the file
        except Exception as e:
            print(f"Failed to execute program: {e}")
            return None

        # Step 2: Wait for the new histogram file
        if not self.wait_for_new_file(histo_file):
            return None

        # Step3: Process the histogram file to calculate FWHM
        try:
            counts = np.loadtxt(histo_file)
            print(f"Loaded {len(counts)} bins from {file_path}")
            fwhm = calculate_fwhm(counts, plot=True)
            if fwhm is not None:
    ...
    def beam_off(self):
        while not self.stop_thread:
            time_since_last_iteration = time.time() - self.last_iteration_time
            if time_since_last_iteration > self.timeout_interval:
                print("Main thread iteration has stopped. Exiting background thread.")
                break
            # This thread will sleep here for a few half-lives
            #time.sleep(self.half_life_P * 50)  # Sleep for a few half-lives     
            time.sleep(6000)  # Sleep for five minutes. This gives time to the optimizer to increase the beam rate.    
            ...
            self.new_rate = self._get_values_internal(["LAUREL_RATEMETER:MONITOR"]) # Background
            ...
            while self.difference <= 0:
                ...
                self.new_rate = self._get_values_internal(["LAUREL_RATEMETER:MONITOR"])
    ...
    def _update_json_config(self):
        ...
        try:
            ...
            # Update the "good_tune" -> "K0" entry with the best post_dict
            config['good_tune']['PII BUNCHING'] = self.best_post_dict
